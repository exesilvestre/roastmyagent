...
from app.services import llm_provider_service as llm_svc
...
async def list_llm_providers(db: DbSession) -> list[LlmProviderOut]:
    rows = await llm_svc.list_providers(db)
...
async def update_llm_provider(
    ...
    try:
        row = await llm_svc.update_provider(db, provider_id, patch)
    except RuntimeError:
        raise HTTPException(
    ...
    view = await llm_svc.get_provider_public(db, provider_id)
    if view is None:
...
async def activate_llm_provider(provider_id: str, db: DbSession) -> None:
    ok = await llm_svc.activate_provider(db, provider_id)
    if not ok:
        raise HTTPException(
